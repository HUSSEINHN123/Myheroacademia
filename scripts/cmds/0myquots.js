...
module.exports = {
  config: {
    ...
    countDown: 5,
    role: 0,
    shortDescription: "إقتباس من صنعك مرفقة مع صور",
    longDescription: "قم بإنشاء إقتباسات خاصة بك بشكل إحترافي",
    category: "متعة",
  },
  ...
  onStart: async function ({ api, event, args }) {
    ...
    if (input.length !== 2) {
      api.sendMessage(' لكي تستعمل هذا الأمر إستعمل ببساطة “إقتباساتي [نص] = [المؤلف]”...', threadID, messageID);
      return;
    }
    ...
    if (!quoteText || !authorName) {
      api.sendMessage(' ⚠️ |مطلوب كتابة كل من نص الاقتباس واسم المؤلف.', threadID, messageID);
      return;
    }
